...
/** CUP v0.11a beta 20060608 generated parser.
  * @version Sun Jun 13 01:00:13 CEST 2021
  */
  ...
  public final java_cup.runtime.Symbol CUP$parser$do_action(
    ...
    throws java.lang.Exception
    {
      ...
      switch (CUP$parser$act_num)
		...
		String nombre_identificador = ((TokenIF) id).getLexema().toLowerCase();
		SymbolIF variable;
		if(scopeManager.containsSymbol(nombre_identificador.toLowerCase())){
		...
		if(!(((SentenciaFuncion) funcvsproc).getSymbol().getClass().getSimpleName().equals("SymbolProcedure"))){
		...
		System.out.println("Iniciando analisis llamada a funcion: " + ((TokenIF) id).getLexema().toLowerCase());
		String nombre = ((TokenIF) id).getLexema().toLowerCase();
		if(scopeManager.containsSymbol(nombre.toLowerCase())){
			...
			if(simbolo instanceof SymbolFunction){
				...
				if(parametros != null){
					...
					if(parametros.size() == ListaVariables.getVariables().size()){
						...
						while(itpar.hasNext() && itdecl.hasNext()){
							...
							SymbolParameter parametro_declarado = (SymbolParameter) itdecl.next();
							String nombre_parametro = parametro.getNombre();
							String tipo_declarado = parametro_declarado.getType().getName();
							...
							} else if(nombre_parametro != null && !scopeManager.containsSymbol(nombre_parametro.toLowerCase())){
							...
							//Compruebo que si es un parametro por referencia (VAR) solo pueda ser llamado mediante variables.
							if(parametro_declarado.getValVSRef()){
								if(nombre_parametro == null){
									semanticErrorManager.semanticFatalError("Solo se pueden utilizar como parametros por referencia variables, actualmente literal: " + parametro.getTipo());
								}
								if(scopeManager.containsSymbol(nombre_parametro.toLowerCase())){
									if (scopeManager.searchSymbol(nombre_parametro).getClass().getSimpleName().equals("SymbolVariable") || scopeManager.searchSymbol(nombre_parametro).getClass().getSimpleName().equals("SymbolParameter")){
									}
									else
										semanticErrorManager.semanticFatalError("Solo se pueden utilizar como parametros por referencia variables: " + nombre_parametro.toLowerCase() + " tipo " + scopeManager.searchSymbol(nombre_parametro).getClass().getSimpleName());
								}
								else
									semanticErrorManager.semanticFatalError("Solo se pueden utilizar como parametros por referencia variables: " + nombre_parametro.toLowerCase());
							}
							
							//Y ya puedo comprobar si los tipos concuerdan. 
							if(!tipo_declarado.equalsIgnoreCase(tipo_parametro)){
				...
				semanticErrorManager.semanticFatalError("No se ha encontrado declarada una funcion o procedimiento con el nombre: " + nombre);
			...
			System.out.println("Analizando llamada a subprograma de nombre y tipo: " + simbolo.getName() + " : " + simbolo.getClass().getSimpleName());
		...
		if(piz instanceof SymbolBase){
			...
			if((pi.getType().getName().equalsIgnoreCase("entero")) && (pd.getType().equalsIgnoreCase("entero"))){			
				asign = new SentenciaAsignacion(pi, pd, "entero");
				System.out.println("Sentencia de asignación entera analizada");
			} else if(pi.getType().getName().equalsIgnoreCase("boolean") && pd.getType().equalsIgnoreCase("boolean")){
			...
			//Compruebo si concuerdan los tipos de la parte izquierda y derecha y qué tipòs son, según sea seteo la SentenciaAsignacion.
			if((pi.equalsIgnoreCase("entero")) && (pd.getType().equalsIgnoreCase("entero"))){			
				asign = new SentenciaAsignacion(((Array) piz).getSymbol(), pd, "entero");
				System.out.println("Sentencia de asignación entera analizada");
			} else if(pi.equalsIgnoreCase("boolean") && pd.getType().equalsIgnoreCase("boolean")){
		...
		String lexema = ((TokenIF) id).getLexema().toLowerCase();
		SymbolIF identificador;
		//Compruebo y obtengo el sombolo del identificador.
		if(scopeManager.containsSymbol(lexema.toLowerCase())){
			identificador = scopeManager.searchSymbol(lexema);
		...
		ScopeIF scope = scopeManager.getCurrentScope();
		SymbolTableIF syTable = scope.getSymbolTable();
		String nombre_vector = ((TokenIF) id).getLexema().toLowerCase();
		SymbolIF vector = null; 
		TypeArray tipo_array;
		Integer rango = ((PrimitivaAritmetica) prim).getValor();

		if(scopeManager.containsSymbol(nombre_vector.toLowerCase()) && (scopeManager.searchSymbol(nombre_vector.toLowerCase()).getType() instanceof TypeArray)){
			System.out.println("Accediendo a vector: " + nombre_vector);
			vector = scopeManager.searchSymbol(nombre_vector);
			//Compruebo que no se haya pasado de dimensión el rango proporcionado (>=).
			if(rango != null && (((int) rango < ((TypeArray) vector.getType()).getRangoInf()) || ((int) rango > ((TypeArray) vector.getType()).getRangoSup()))){
				semanticErrorManager.semanticFatalError("Intento de acceso al array " + nombre_vector + " fuera de rango, su rangos de valores son " + ((TypeArray) vector.getType()).getRangoInf() + " y " + ((TypeArray) vector.getType()).getRangoSup() + "; inteto de acceso a " + rango);
			}
		...
		icb.addQuadruple("SUB", temp_p, temp_p, ((TypeArray) vector.getType()).getRangoInf());
		...
		if(va instanceof Parametro){
			semanticErrorManager.semanticFatalError("No se ha declarado con anterioridad el identificador: " + ((Parametro) va).getNombre());
		}
		...
		if(vd instanceof Parametro){
			semanticErrorManager.semanticFatalError("No se ha declarado con anterioridad el identificador: " + ((Parametro) vd).getNombre());
		}
		...
		if(vi instanceof Parametro){
			semanticErrorManager.semanticFatalError("No se ha declarado con anterioridad el identificador: " + ((Parametro) vi).getNombre());
		}
		if(vd instanceof Parametro){
			semanticErrorManager.semanticFatalError("No se ha declarado con anterioridad el identificador: " + ((Parametro) vd).getNombre());
		}
		...
		icb.addQuadruple("MVM", ((Array) v).getTemporal(), temp_act);
		...
		if(scopeManager.searchSymbol(funcion.getName()).getClass().getSimpleName().equals("SymbolProcedure")){
			semanticErrorManager.semanticFatalError("No esta permitido el uso de procedimientos como parte de una expresion: " + funcion.getName() + " : " + scopeManager.searchSymbol(funcion.getName()).getClass().getSimpleName());
		}
		...
		//Compruebo que estemos en una declaracion de variables, en dicho caso si existe la variable es un error.
		if(ListaVariables.getEnDeclaracion()){
			if(scopeManager.getCurrentScope().getSymbolTable().containsSymbol(lexema.toLowerCase())){
				ListaVariables.setEnDeclaracion(false);
				semanticErrorManager.semanticFatalError(" Ya existe una variable en este ámbito con el mismo nombre: " + lexema);
			}
			...
			//Compruebo y obtengo el sombolo del identificador.
			if(scopeManager.containsSymbol(lexema.toLowerCase())){
				identificador = scopeManager.searchSymbol(((TokenIF) id).getLexema().toLowerCase());
				System.out.println("Identificador encontrado: " + ((TokenIF) id).getLexema().toLowerCase() + " de clase " + identificador.getClass().getSimpleName() + " y tipo " + identificador.getType().getName());
				...
				switch(identificador.getClass().getSimpleName()){
					case "SymbolConstant":
						if(identificador.getType().getName().equalsIgnoreCase("entero")){
						...
						else if(identificador.getType().getName().equalsIgnoreCase("boolean")){
					...
					case "SymbolParameter":
						if(identificador.getType().getName().equalsIgnoreCase("entero")){
							...
							if(((TypeArray) identificador.getType()).getTypeSimple().getName().equalsIgnoreCase("entero")){
								//Creo la primitiva en el caso de que el tipo de la constante sea entera.
								primitiva = new PrimitivaAritmetica(null,identificador.getType().getName(),null); 
								//primitiva.setValor(Integer.parseInt(identificador.getValue()));
								((PrimitivaAritmetica) primitiva).setNombre(lexema);
							} 
							else if(((TypeArray) identificador.getType()).getTypeSimple().getName().equalsIgnoreCase("boolean")){
								//HAY QUE IMPLEMENTAR getValue() en simbolos variables.
				...
				//Puede que sea un identificador que se esté declarando justamente ahora.
				semanticErrorManager.semanticFatalError("No se ha encontrado declarado el simbolo con nombre: " + lexema);
			}
		}
		...
		if(!SymbolFunction.getDevolverDeclarado()){
			semanticErrorManager.semanticFatalError("No se ha explicitado un devolver en la función: " + ((TokenIF) id).getLexema().toLowerCase());
		} else if(!SymbolFunction.comprobarTiposDevolver((String) retorno)){
			SymbolFunction.clearTiposDevolver();
			semanticErrorManager.semanticFatalError("Al menos un tipo devolver explicitado en la función " + ((TokenIF) id).getLexema().toLowerCase() + " no es del tipo declarado: " + (String) retorno);
		} else
		...
		if(parametros != null){
			for(SymbolParameter parametro : (ArrayList<SymbolParameter>) parametros) {
				if(parametro.getType().getClass().getSimpleName().equals("TypeArray") && !parametro.getValVSRef()){
					semanticErrorManager.semanticFatalError("Los vectores solo pueden ser parametros por referencia, falta VAR para el vector: " + parametro.getName());
				}
				//Los añado a la tabla de simbolos
				syTable.addSymbol(parametro);
			}
		}
		System.out.println("Parametros de la funcion " + ((TokenIF) id).getLexema().toLowerCase() + "añadidos a la tabla de simbolos");
		...
		SymbolFunction funcion = new SymbolFunction(scope, ((TokenIF) id).getLexema().toLowerCase(), (TypeIF) scopeManager.searchType(((String) retorno).toLowerCase()));
		...
		System.out.println("Funcion añadida a la tabla de simbolos: " + ((TokenIF) id).getLexema().toLowerCase());
		...
		if(scopeManager.containsSymbol(((TokenIF) id).getLexema().toLowerCase())){
			semanticErrorManager.semanticFatalError("Ya existe un simbolo con este nombre de funci�n: " + ((TokenIF) id).getLexema().toLowerCase());
		}
		
		//Abro una nueva scope para añadir la información de la función.
		String nombre_funcion = ((TokenIF) id).getLexema().toLowerCase();
		scopeManager.openScope(nombre_funcion.toLowerCase());
		...
		System.out.println("Parametros del procedimiento " + ((TokenIF) id).getLexema().toLowerCase() + "añadidos a la tabla de simbolos");
		//Guardo el procedimiento en el scope padre.
		scope = scopeManager.getParentScope();
		syTable = scope.getSymbolTable();
		SymbolProcedure procedimiento = new SymbolProcedure(scope, ((TokenIF) id).getLexema().toLowerCase(), null);
		...
		if(scopeManager.containsSymbol(((TokenIF) id).getLexema().toLowerCase())){
			semanticErrorManager.semanticFatalError("Ya existe un simbolo con este nombre de procedimiento: " + ((TokenIF) id).getLexema().toLowerCase());
		}
		
		String nombre_procedimiento = ((TokenIF) id).getLexema().toLowerCase();
		//Creo un scope nuevo para añadir la información del procedimiento.
		scopeManager.openScope(nombre_procedimiento.toLowerCase());
		...
		String nombre_tipo = ((TokenIF) id).getLexema().toLowerCase();
		...
		if(scopeManager.searchType(nombre_tipo) instanceof TypeArray)
			semanticErrorManager.semanticFatalError("No se puede crear una función con un tipo de retorno de tipo vector");
		...
		for(Parametro elemento : elementos){
			...
			if((boolean) puntocoma){
				if(syTable.containsSymbol(nombre_variable.toLowerCase())){
					ListaVariables.setEnDeclaracion(false);
				...
				syTable.addSymbol(nombre_variable.toLowerCase(), variable);
			} else {
				//En este caso estamos hablando de una declaración de parametros dentro de una declaración de función.
				SymbolParameter parametro = new SymbolParameter(scope, nombre_variable.toLowerCase(), tipe);	
				//Si la variable declarada ha tenido un var precediendole entonces es por referencia y le seteo dicha información.
				if(elemento.getValVSRef()){
		...
		ListaVariables.clearVariables();
		//Seteo inicio de declaracion
		ListaVariables.setEnDeclaracion(true);
		...
		System.out.println("Obteniedno lexema de declaración de variable: " + ((TokenIF) id).getLexema().toLowerCase());
		String lexema = ((TokenIF) id).getLexema().toLowerCase();
		//Creo el parámetro.
		...
		RESULT = ((TokenIF) id).getLexema().toLowerCase();
		...
		if(mv instanceof PrimitivaAritmetica) {
			...
			icb.addQuadruple("MVM", temp_r, temp_act);
		...
		ScopeIF scope = scopeManager.getCurrentScope();
		SymbolTableIF syTable = scope.getSymbolTable();
		String nombre_rango = ((TokenIF) id).getLexema().toLowerCase();
		//Compruebo que el identificador exista con anterioridad.
		if(!scopeManager.containsSymbol(nombre_rango.toLowerCase())){
		...
		if(!rango.getType().getName().equalsIgnoreCase("entero")){
		...
		//Creo el código intermedio.
		Variable variable = new Variable(nombre_rango, scope);
		icb.addQuadruple("MV", temp_act, variable);
		...
		if(!scope.getScopeManager().containsType(((String) tipo))){
			if(((String)tipo).equalsIgnoreCase("entero")){
				tipe = new TypeSimple(scope, "entero");
				tyTable.addType(tipe);
			} else if(((String)tipo).equalsIgnoreCase("boolean")){
				tipe = new TypeSimple(scope, "boolean");
				tyTable.addType(tipe);
			} else {
		...
		tipo_usuario.setRangoSup((int) rango_sup);
		tipo_usuario.setRangoInf((int) rango_inf);
		...
		String nombre_constante = ((TokenIF) id).getLexema().toLowerCase();
		...
		if(valor instanceof PrimitivaAritmetica){
			...
			if(syTable.containsSymbol(nombre_constante.toLowerCase())){
			...
			syTable.addSymbol(nombre_constante.toLowerCase(), constante);
