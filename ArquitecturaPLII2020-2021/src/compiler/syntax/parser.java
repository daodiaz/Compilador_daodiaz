...
/** CUP v0.11a beta 20060608 generated parser.
  * @version Wed May 12 00:13:28 CEST 2021
  */
    ...
    throws java.lang.Exception
    {
      ...
      switch (CUP$parser$act_num)
		...
		//En el caso de miembros de vectores la cosa se complica un poco, compruebo que primero sea un array y luego que su tiempo simple sea igual a la expresión de la derecha.
			...
			asign = new SentenciaAsignacion((SymbolIF) pi, (Expresion) pd,"miembro_vector");
		...
		icb.addQuadruple("MV", temp_act, pd_temp);
		...
		if(pi instanceof SymbolVariable)
			((SymbolVariable) pi).setIsInit(true);
		RESULT = asign;
		...
		if(mv instanceof PrimitivaAritmetica){
			...
			semanticErrorManager.semanticFatalError("En este punto sólo se espera la llegada de una primitiva aritmética o lógica");
			lexema = null;
		}
		SymbolIF vector;
...
.getRango() + " y se trata de acceder al miembro " + (int) rango);
		...
		switch(tipo_array.getTypeSimple().getName()){
			...
			case "boolean": 
				...
				break;
			default:
				semanticErrorManager.semanticFatalError("Los vectores solo pueden estar conformados de enteros o lógicos, actualmente: " + tipo_array.getTypeSimple());
		}
		...
		SymbolFunction funcion = null;
		//Compruebo y obtengo la función de la tabla de símbolos.
		if(scopeManager.containsSymbol(lexema)){
			funcion = (SymbolFunction) scopeManager.searchSymbol(lexema);
		}
		else
			semanticErrorManager.semanticFatalError("No existe ninguna función con nombre " + lexema);
		
		SentenciaFuncion sent_funcion = new SentenciaFuncion(funcion);
		...
		switch(funcion.getReturn().getName()){
			...
			case "boolean": 
				...
				break;
			default:
				semanticErrorManager.semanticFatalError("Las funciones con este tipo de retorno " + funcion.getType().getName() + " no pueden formar parte de una expresión.");		
		}
